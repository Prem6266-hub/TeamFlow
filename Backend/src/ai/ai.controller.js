    ...
    for (const task of result.tasks) {
      ...
      if (!assignedMember && normalizedAssignedTo) {
        assignedMember = teamMembers.find((member) => {
          ...
          return (
            normalizedAssignedTo.includes(memberName) ||
            memberName.includes(normalizedAssignedTo)
          );
        });
      }
      ...
      console.log("Saving task for:", assignedMember.id);
  ...
  try {
    ...
    if (!workspace) {
      return res.status(404).json({
        message: "Workpace not found",
      });
    }

    const isOwner = workspace.owner?._id?.toString() === currentUserId;
...
module.exports = { generateTasks, chatWithAi };
